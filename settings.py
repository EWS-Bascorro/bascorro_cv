    ...
    def _load_setting(self) -> dict:
        try:
            with open(self._filename) as setting:
                logger.debug(f"Opening {self._filename}")
                return json.load(setting)
        except FileNotFoundError:
            logger.info(f"{self._filename} not found, creating new file")
            with open(self._filename, "w+") as setting:
